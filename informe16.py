...
def ver_informe():
    ...
    contenido_html = contenido_html.replace('Detalle de la notificación', resumen_detallado)

    # Mostrar el informe detallado en un cuadro de diálogo de mensaje
    messagebox.showinfo("Informe Detallado", contenido_html)
def enviar_correos():
    if not verificar_archivo():
        return
    ...
    for correo, grupo in grupo_correo:
        ...
        # Crear el objeto MIMEImage con la imagen codificada en base64
        imagen_adjunta = MIMEImage(img_data_base64, name=os.path.basename(ruta_imagen))
...
root = tk.Tk()
root.title("Informeand Send Emails")
